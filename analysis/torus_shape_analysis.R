...
data$LGI_tor <- log(data$GI_Tor)
data$LHF_tor <- log(data$HF_Tor)
head(data)
# Import model 
...
model3 <- glm.nb(formula = seedlings ~ ABN + I(LGI_tor^2) + LHF_tor + log(DBH) + 
                   ABN:I(LGI_tor^2) + ABN:LHF_tor + I(LGI_tor^2):LHF_tor, data = data)
summary(model3)
...
pred1 <- expand.grid(ABN = round(quantile(data$ABN, 
                     ...
                     LHF_tor = round(quantile(data$LHF_tor, 
                                          c(0.1,0.25,0.5,0.75,0.9)),1)[3],
                     LGI_tor = seq(min(data$LGI_tor),max(data$LGI_tor),length=50))
...
p2 <- ggplot(pred1, aes(x=LGI_tor, exp(seedlings), fill = Abundance)) + geom_line()  + 
...
ggsave(p2, file = "./graphs/torus_model.png", 
       width = 4, height = 4)
